...
unsigned int nStakeMinAge = 60 * 60 * 24 * 30; //30 days minimum age for coin age
unsigned int nStakeMaxAge = 60 * 60 * 24 * 90; //90 days stake age of full weight
unsigned int nStakeTargetSpacing = 10; // 10 seconds block spacing
const int64 nChainStartTime = 1396664179; 
const int64 nTestNetStartTime = nChainStartTime; 
int nCoinbaseMaturity = 80; // minted blocks need need 80 confirm
...
const string strMessageMagic = "BitcoinPlus Signed Message:\n";
...
int64 GetProofOfStakeReward(int64 nCoinAge, unsigned int nBits, unsigned int nTime)
{
    ...
    // BitcoinPlus: reward for coin-year is cut in half every 64x multiply of PoS difficulty
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal BitcoinPlus miner
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    bool fEnforceBIP30 = true; // Always active in BitcoinPlus
    bool fStrictPayToScriptHash = true; // Always active in BitcoinPlus
    ...
    // ppcoin: fees are not collected by miners as in BitcoinPlus
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // --- patch start//Patch from XertroV, great thanks to him https://bitcointalk.org/index.php?action=profile;u=37740
        // blacklist
        if (GetHash() == uint256("f050fcfec062b91f07c50564163cd9291bb6aab2e4df56cb38ae7680ad8b16a1"))
            return error("CheckBlock() : hash == f050fcfec062b91f07c50564163cd9291bb6aab2e4df56cb38ae7680ad8b16a1");
        // --- patch end
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BitcoinPlus", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
// Genesis block
        const char* pszTimestamp = "OBAMACARE IN JAIL Prisoners get coverage as states shift cost to feds.";
        CTransaction txNew;
        ...
        txNew.strTxComment = "text:BitcoinPlus genesis block";
        ...
        assert(block.hashMerkleRoot == uint256("5578166c10bea0c4649ea2fea5ba6d32b3951904629c09af1e74b135ce05b08f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf4, 0xf9, 0xf9, 0xf8 };
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    unsigned int nBlockPrioritySize = GetArg("-blockprioritysize", 1000);
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    if (hash > hashTarget && pblock->IsProofOfWork())
        return error("BitcoinPlusMiner : proof-of-work not meeting target");

    //// debug print
    printf("BitcoinPlusMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BitcoinPlusMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("BitcoinPlusMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadBitcoinPlusMiner(void* parg);

static bool fGenerateBitcoinPluss = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void BitcoinPlusMiner(CWallet *pwallet, bool fProofOfStake)
{
    printf("CPUMiner started for proof-of-%s\n", fProofOfStake? "stake" : "work");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("BitcoinPlus-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateBitcoinPluss || fProofOfStake)
    {
        if (fShutdown)
            return;
        while (vNodes.empty() || IsInitialBlockDownload())
        {
            ...
            if (fShutdown)
                return;
            if ((!fGenerateBitcoinPluss) && !fProofOfStake)
                return;
        }
        ...
        printf("Running BitcoinPlusMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void static ThreadBitcoinPlusMiner(void* parg)
{
    ...
    try
    {
        ...
        BitcoinPlusMiner(pwallet, false);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadBitcoinPlusMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadBitcoinPlusMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadBitcoinPlusMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateBitcoinPluss(bool fGenerate, CWallet* pwallet)
{
    fGenerateBitcoinPluss = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateBitcoinPluss = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d BitcoinPlusMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!NewThread(ThreadBitcoinPlusMiner, pwallet))
                printf("Error: NewThread(ThreadBitcoinPlusMiner) failed\n");
            Sleep(10);
        }
    }
}
